...
router.get('/financial', async (req, res) => {
  try {
    ...
    // Calculate total salary expenses EXCLUDING owners
    ...
    // Get owner salary total separately for reporting
    const [ownerSalaryExpenses] = await db.query(`
      SELECT COALESCE(SUM(s.Total_Salary), 0) AS total 
      FROM salary s
      JOIN employees e ON s.Employee_ID = e.id
      WHERE s.Payment_Date BETWEEN ? AND ?
    ...
    const ownerPayments = parseFloat(ownerSalaryExpenses[0].total || 0);
    ...
    res.status(200).json({
      ...
      summary: {
        ...
        owner_payments: ownerPayments, // Add this to show owner payments separately
        profit,
        profit_margin: profitMargin.toFixed(2) + '%'
      },
      expense_breakdown: {
        inventory: parseFloat(inventoryExpenses[0].total_expenses || 0),
        employee_salaries: parseFloat(salaryExpenses[0].total || 0), // Renamed to be clear these are non-owner salaries
        owner_withdrawals: ownerPayments // Add owner payments as a separate category
      },
