...
// Get current month purchase items count
router.get('/current-month-purchases', async (req, res) => {
  try {
    ...
    // Query to get the count of purchase items for the current month
    ...
    // Query to get the purchase items grouped by inventory ID for the current month
    const [purchasesByInventory] = await db.query(`
      SELECT 
        p.Inventory_ID,
        i.Item_Name,
    ...
    res.status(200).json({
      ...
      purchaseCount: purchaseCountResult[0].totalItems || 0,
      totalQuantity: purchaseCountResult[0].totalQuantity || 0,
      totalSpent: purchaseCountResult[0].totalSpent || 0,
      topPurchasedItems: purchasesByInventory
    });
  } catch (error) {
    console.error('Error fetching current month purchases:', error);
    res.status(500).json({ error: 'Failed to fetch current month purchase data' });
  }
});
