...
// Get all inventory items with total batch quantity and stock status
router.get('/inventory-status/all', async (req, res) => {
    try {
        // Query to fetch inventory details and total quantity from InventoryBatch
        const [inventory] = await db.query(`
            SELECT 
                i.Inventory_ID,
                i.product_name,
                i.Stock_Limit,
                COALESCE(SUM(ib.Quantity), 0) AS totalQuantity
            FROM Inventory i
            LEFT JOIN InventoryBatch ib ON i.Inventory_ID = ib.Inventory_ID
            GROUP BY i.Inventory_ID, i.product_name, i.Stock_Limit
        `);
        ...
        // Add stock status for each inventory item
        const inventoryWithStatus = inventory.map(item => {
            let stockStatus;

            if (item.totalQuantity == 0) {
                stockStatus = "Out of Stock";
            } else if (item.totalQuantity <= item.Stock_Limit) {
                stockStatus = "Buy Items";
            } else {
        ...
        // Return all inventory items with stock status
